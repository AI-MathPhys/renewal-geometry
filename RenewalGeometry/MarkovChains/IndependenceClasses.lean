...
def _root_.NCG.Multigraph.Walk.fwdCount (e₀ : G.E) : ∀ {u v : G.V}, G.Walk u v → ℕ
  | _, _, .nil _ => 0
...
def _root_.NCG.Multigraph.Walk.bwdCount (e₀ : G.E) : ∀ {u v : G.V}, G.Walk u v → ℕ
  | _, _, .nil _ => 0
...
def _root_.NCG.Multigraph.Walk.usesCount (e₀ : G.E) {u v : G.V} (p : G.Walk u v) : ℕ :=
  p.fwdCount e₀ + p.bwdCount e₀

open NCG.Multigraph.Walk in
/-- The affinity of the indicator cochain of `e₀` counts signed
traversals of `e₀`. -/
theorem _root_.NCG.Multigraph.Walk.affinity_indicator (e₀ : G.E) :
...
theorem _root_.NCG.Multigraph.Walk.holonomy_indicator (e₀ : G.E) :
    ∀ {u v : G.V} (p : G.Walk u v),
...
`[A] = 0 / ≠ 0` and `[χ] = 0 / ≠ 0`. -/
theorem _root_.NCG.Multigraph.independence_of_classes {v : G.V} {e₀ : G.E}
    (p : G.Walk v v) (hone : p.usesCount e₀ = 1) :
...
theorem _root_.NCG.Multigraph.detailed_balance_indep_orientability {v : G.V} {e₀ : G.E}
    (p : G.Walk v v) (hone : p.usesCount e₀ = 1) :
    (∃ (w : G.E → ℝ) (L : G.E → Matrix (Fin 1) (Fin 1) ℝ),
      (∀ e, 0 < w e) ∧ (∀ e, (L e).det ≠ 0)
        ∧ H1R.mk G (fun e => Real.log (w e)) = 0
