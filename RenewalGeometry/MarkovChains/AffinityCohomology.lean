...
namespace RenewalGeometry.Multigraph
open NCG.Multigraph

open NCG RenewalGeometry
...
def _root_.NCG.Multigraph.coboundR : (G.V → ℝ) →ₗ[ℝ] (G.E → ℝ) where
...
open NCG.Multigraph in
@[simp]
theorem _root_.NCG.Multigraph.coboundR_apply (u : G.V → ℝ) (e : G.E) :
    coboundR G u e = u (G.tgt e) - u (G.src e) := rfl

open NCG.Multigraph in
/-- Real first cohomology `H¹(G;ℝ) = C¹/δC⁰` (a graph has no
2-cells). -/
def _root_.NCG.Multigraph.H1R := (G.E → ℝ) ⧸ LinearMap.range (coboundR G)
...
open NCG.Multigraph.H1R in
theorem _root_.NCG.Multigraph.H1R.mk_eq_zero_iff {A : G.E → ℝ} :
...
namespace Walk
open NCG.Multigraph.Walk

open NCG.Multigraph.Walk in
...
def _root_.NCG.Multigraph.Walk.affinity (A : G.E → ℝ) : ∀ {u v : G.V}, G.Walk u v → ℝ
  | _, _, .nil _ => 0
  | _, _, .fwd e p => A e + p.affinity A
  | _, _, .bwd e p => -A e + p.affinity A

open NCG.Multigraph.Walk in
@[simp] theorem _root_.NCG.Multigraph.Walk.affinity_nil (A : G.E → ℝ) (v : G.V) :
    (Walk.nil v).affinity A = 0 := rfl

open NCG.Multigraph.Walk in
@[simp] theorem _root_.NCG.Multigraph.Walk.affinity_fwd (A : G.E → ℝ) (e : G.E) {w : G.V}
    (p : G.Walk (G.tgt e) w) :
    (Walk.fwd e p).affinity A = A e + p.affinity A := rfl

open NCG.Multigraph.Walk in
@[simp] theorem _root_.NCG.Multigraph.Walk.affinity_bwd (A : G.E → ℝ) (e : G.E) {w : G.V}
    (p : G.Walk (G.src e) w) :
...
under concatenation. -/
theorem _root_.NCG.Multigraph.Walk.affinity_append (A : G.E → ℝ) :
...
theorem _root_.NCG.Multigraph.Walk.affinity_reverse (A : G.E → ℝ) :
...
theorem _root_.NCG.Multigraph.Walk.affinity_gauge (A : G.E → ℝ) (u : G.V → ℝ) :
...
theorem _root_.NCG.Multigraph.Walk.affinity_add_cochain (A B : G.E → ℝ) {u v : G.V}
    (p : G.Walk u v) :
...
theorem _root_.NCG.Multigraph.Walk.affinity_zero {u v : G.V} (p : G.Walk u v) :
...
theorem _root_.NCG.Multigraph.Walk.affinity_congr {A A' : G.E → ℝ}
...
theorem _root_.NCG.Multigraph.H1R.mk_eq_zero_iff_cycles {v₀ : G.V}
...
theorem _root_.NCG.Multigraph.H1R.mk_eq_zero_iff_gauge (A : G.E → ℝ) :
...
noncomputable def _root_.NCG.Multigraph.unitSign (r : ℝˣ) : ZMod 2 :=
  if 0 < (r : ℝ) then 0 else 1

open NCG.Multigraph in
theorem _root_.NCG.Multigraph.unitSign_mul (r s : ℝˣ) :
...
open NCG.Multigraph in
theorem _root_.NCG.Multigraph.unitSign_inv (r : ℝˣ) : unitSign r⁻¹ = unitSign r := by
  unfold unitSign
...
open NCG.Multigraph in
theorem _root_.NCG.Multigraph.lineSign_eq_unitSign (lam : G.E → ℝˣ) (e : G.E) :
...
theorem _root_.NCG.Multigraph.lineSign_gauge (g : G.V → ℝˣ) (lam : G.E → ℝˣ) :
...
theorem _root_.NCG.Multigraph.unit_eq_of_sign_mag (r s : ℝˣ)
...
noncomputable def _root_.NCG.Multigraph.unitOf (a : ℝ) (ε : ZMod 2) : ℝˣ :=
  (if ε = 0 then 1 else -1) * Units.mk0 (Real.exp a) (Real.exp_ne_zero a)

open NCG.Multigraph in
theorem _root_.NCG.Multigraph.zmod2_cases (x : ZMod 2) : x = 0 ∨ x = 1 := by
  revert x
  decide

open NCG.Multigraph in
theorem _root_.NCG.Multigraph.unitOf_val (a : ℝ) (ε : ZMod 2) :
    (unitOf a ε : ℝ) = (if ε = 0 then 1 else -1) * Real.exp a := by
  unfold unitOf
  rcases zmod2_cases ε with h | h <;> subst h <;> simp

open NCG.Multigraph in
theorem _root_.NCG.Multigraph.unitOf_log_abs (a : ℝ) (ε : ZMod 2) :
...
open NCG.Multigraph in
theorem _root_.NCG.Multigraph.unitOf_sign (a : ℝ) (ε : ZMod 2) :
...
theorem _root_.NCG.Multigraph.line_realization (A : G.E → ℝ) (χ : G.E → ZMod 2) :
    ∃ lam : G.E → ℝˣ,
...
theorem _root_.NCG.Multigraph.line_classification (lam lam' : G.E → ℝˣ) :
...
theorem _root_.NCG.Multigraph.orientation_class_zero_iff_coherent (χ : G.E → ZMod 2) :
...
holonomy. -/
theorem _root_.NCG.Multigraph.orientation_class_zero_iff_cycles {v₀ : G.V}
...
noncomputable def _root_.NCG.Multigraph.orientationCoverSplit (χ : G.E → ZMod 2)
...
theorem _root_.NCG.Multigraph.signedCover_connected_of_ne_zero {v₀ : G.V}
    (hconn : G.ConnectedTo v₀) {χ : G.E → ZMod 2}
    (hne : H1.mk G χ ≠ 0) :
...
theorem _root_.NCG.Multigraph.signed_cover_bridge (lam : G.E → ℝˣ) (r : G.E → ℝ)
    (hcal : ∀ e, |(lam e : ℝ)| = r e) :
...
theorem _root_.NCG.Multigraph.positive_comparison_trivial_sign (lam : G.E → ℝˣ)
    (hpos : ∀ e, 0 < (lam e : ℝ)) :
...
class. -/
theorem _root_.NCG.Multigraph.addHom_real_zmod2_eq_zero (f : ℝ →+ ZMod 2) : f = 0 := by
  ext x
...
theorem _root_.NCG.Multigraph.coboundR_bouquet_one :
...
open NCG.Multigraph in
theorem _root_.NCG.Multigraph.H1R_bouquet_mk_eq_zero_iff (A : (bouquet 1).E → ℝ) :
...
open NCG.Multigraph in
theorem _root_.NCG.Multigraph.H1_bouquet_mk_eq_zero_iff (χ : (bouquet 1).E → ZMod 2) :
...
independent. -/
theorem _root_.NCG.Multigraph.independence_one_loop (a : ℝ) (ha : a ≠ 0) :
...
theorem _root_.NCG.Multigraph.three_levels_distinct :
    (∃ lam : (bouquet 1).E → ℝˣ,
...
theorem _root_.NCG.Multigraph.no_invariant_selector {H Q : Type*} (q : H → Q) (α : H → H)
...
end RenewalGeometry.Multigraph
