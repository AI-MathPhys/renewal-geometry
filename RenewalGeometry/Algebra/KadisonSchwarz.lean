...
(`RenewalGeometry.IsArchimedeanStarOrder`) — satisfied by every C⋆-algebra — which we
isolate as a hypothesis and verify for `ℂ`.

## Main results

* `RenewalGeometry.matrixQF_pair` — the Gram matrix of a pair is quadratic-form
  positive;
* `RenewalGeometry.IsCompletelyPositive.map_star` — CP maps are star-preserving;
* `RenewalGeometry.IsCompletelyPositive.isSchwarzMap` — **Kadison–Schwarz**: a unital
  CP map satisfies `φ(x)⋆ φ(x) ≤ φ(x⋆ x)`;
* `RenewalGeometry.IsArchimedeanStarOrder`, `RenewalGeometry.isArchimedeanStarOrder_complex` —
  the Archimedean property, verified for `ℂ`;
* `RenewalGeometry.multiplicative_of_schwarz_eq` — **Choi's theorem**: equality in the
  Schwarz inequality at `x` gives `φ(x⋆ w) = φ(x)⋆ φ(w)` for all `w`;
* `RenewalGeometry.UCPMap.unit_map_star`, `RenewalGeometry.UCPMap.unit_map_mul` — the
...
Archimedean property of `ℝ`); `RenewalGeometry.isArchimedeanStarOrder_complex` verifies
