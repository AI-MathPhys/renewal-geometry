...
* `RenewalGeometry.IsSchwarzMap` — the Kadison–Schwarz inequality
  `φ(x)⋆ φ(x) ≤ φ(x⋆ x)`;
* `RenewalGeometry.IsPositiveMap.mono` — positive linear maps are monotone;
* `RenewalGeometry.schwarz_sandwich_eq` — the **sandwich equality**: if `Θ` is a
