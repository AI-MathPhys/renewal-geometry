...
theorem ennrealJointCommutatorEnergy_matrixL2
    {n : Type*} [Fintype n] {s : ℕ}
    (c : Fin s → Matrix n n ℂ) (X : Matrix n n ℂ) :
    RenewalGeometry.VaryingHilbert.ennrealBoundedOperatorEnergy
      (jointCommutatorCLM c) (matrixL2 X) =
        ENNReal.ofReal
          (∑ j, (((c j * X - X * c j)ᴴ *
            (c j * X - X * c j)).trace).re) := by
  rw [RenewalGeometry.VaryingHilbert.ennrealBoundedOperatorEnergy,
...
theorem jointCommutator_resolventEquation_of_laplacianEquation
    ...
    RenewalGeometry.VaryingHilbert.OperatorGraphResolventEquation
      (⊤ : Submodule ℂ (EuclideanSpace ℂ (n × n)))
      (RenewalGeometry.VaryingHilbert.boundedOperatorGraphMap (jointCommutatorCLM c))
      lam f x := by
  apply RenewalGeometry.VaryingHilbert.boundedOperatorGraph_resolventEquation_of_normalEquation
