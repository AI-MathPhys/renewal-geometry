...
* **generator exchange calculus**: moving a generator across a product of
  ...
  (`RenewalGeometry.gen_mul_prod`), and products over disjoint index lists exchange
  with the bilinear sign `(−1)^{k₁k₂}` (`RenewalGeometry.prod_mul_prod_disjoint`) —
...
* **the Gram matrix** `Ω = Jₙ − Iₙ` of the generator basis (all-ones off
  ...
  even**: for even `n` it is an involution (`RenewalGeometry.gramMatrix_sq`), for odd
  `n` the all-ones vector lies in its kernel
  (`RenewalGeometry.gramMatrix_not_isUnit_of_odd`).  This is the rank computation that
