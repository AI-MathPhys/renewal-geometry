...
(`RenewalGeometry.blockRG_eigenvalue`).  For the volume-dual response of degree
`k = d − 2` the eigenvalue is `λ_d = b^{3−d}`.

**Corollary `cor:relevance`** (relevance trichotomy): with `b ≥ 2`,

* `d = 3` — marginal/principal: `λ = 1` (`RenewalGeometry.relevance_marginal`);
* `d ≥ 5` — irrelevant: `λ < 1` (`RenewalGeometry.relevance_irrelevant`);
* `d = 1` — no bivector domain: every alternating bilinear form on a
  one-dimensional space vanishes (`RenewalGeometry.alternating_rank_one_eq_zero`).
