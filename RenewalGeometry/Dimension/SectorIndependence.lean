...
intersect trivially (`RenewalGeometry.symm_inter_antisymm_eq_zero`), every matrix
splits uniquely into its symmetric and antisymmetric parts
(`RenewalGeometry.symm_add_antisymm`), and for rank `≥ 2` both sectors are nonzero
(`RenewalGeometry.exists_symm_ne_zero`, `RenewalGeometry.exists_antisymm_ne_zero`) — so neither
