...
identity (`RenewalGeometry.isotropy_not_derived`): weight the first axis twice as
much as the others.
...
Clifford grade (`RenewalGeometry.engineering_coupling_dimension`).  The `d = 3`
selection is the Hodge/Clifford-grade matching
(`RenewalGeometry.closure_selects_three`), not RG relevance. -/
...
`RenewalGeometry.closure_selects_three`. -/
