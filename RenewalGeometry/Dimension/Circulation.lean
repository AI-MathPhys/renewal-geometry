...
* `RenewalGeometry.pairResponse_eq_antisym` — `b_ren[ν] = Γ(Ξ(ν))`: the response
  equals its antisymmetrisation (the oriented-circulation form);
* `RenewalGeometry.pairResponse_eq_zero_of_symm` — a parity (time-reversal)
  symmetric law (`p_{ab} = p_{ba}`) has vanishing response, **regardless
  of dimension**;
* `RenewalGeometry.circulation`, `RenewalGeometry.circulation_eq_zero_of_symm` — the oriented
