...
(`RenewalGeometry.psd_zero_diag_null`), so if no nonzero vector has zero variance
the form has trivial kernel and full rank
(`RenewalGeometry.covariance_full_rank`).  The Markov-additive CLT identifying the
