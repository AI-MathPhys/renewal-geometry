...
* **Proposition `prop:additive-access-main`** (additive access functional):
  ...
  (`RenewalGeometry.factored_access_const`), the replicated allocation runs away
  linearly (`RenewalGeometry.replicated_access_tendsto_atTop`), and the additive
  allocation has the finite interior optimum
  `∏ Wᵢ ≤ (e^B/m)^m` attained at the isotropic allocation
  (`RenewalGeometry.additive_allocation_le`, `RenewalGeometry.additive_allocation_eq`) — the
...
* **Lemma `lem:odd-access-fixed-points`**: the access self-consistency
  ...
  (`RenewalGeometry.odd_access_fixed_points`).  In fact every rank `d ≥ 5` — odd or
  not — fails to be a fixed point (`RenewalGeometry.not_accessFixedPoint_of_five_le`,
...
* **Theorem `thm:conditional-access-3plus1`**: under the access
  ...
  fixed point is `d = 3` (`RenewalGeometry.conditional_access_selection`).
-/
