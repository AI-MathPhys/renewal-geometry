...
* `RenewalGeometry.even_finrank_of_isAlt_nondegenerate'` — the induction;
* `RenewalGeometry.even_finrank_of_isAlt_nondegenerate` — `Even (finrank K V)`;
* `RenewalGeometry.not_nondegenerate_of_isAlt_of_odd_finrank` — odd-dimensional spaces
  carry no nondegenerate alternating form;
* `RenewalGeometry.no_two_plus_one` — the `2+1` exclusion over `𝔽₂`
  (`thm:minimal-nondegenerate-3plus1` (ii));
* `RenewalGeometry.odd_spatial_rank_of_isAlt_nondegenerate` — the spatial rank
