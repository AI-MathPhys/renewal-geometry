...
* **Structure-group reduction** (`RenewalGeometry.structure_group_rank_two`): on a
  ...
  permutation lemma (`RenewalGeometry.conePositive_perm`) to `n = 2`, where the only
...
* **The two-cochain calculus** on the model fibre `R²` with
  ...
  `S^{α+β} = S^α S^β` (`RenewalGeometry.spow_add`), the coboundary action of a sheet
  change `c ↦ c + δa` (`RenewalGeometry.gauge_transport` =
  Lemma `lem:two-cochains-one-class` (iii)), and the **gauge-invariant
  homogeneity degree**
  `J_y T_e = (−1)^{ε(e)} T_e J_x` with `ε(e) = c(e) + j(x) + j(y)`
  (`RenewalGeometry.homogeneity_degree` = Lemma `lem:two-cochains-one-class` (iv)),
  whence `[ε] = [c] ∈ H¹(G, ℤ/2)`.
-/
