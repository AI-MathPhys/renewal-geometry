...
`[u] ∈ H¹(G;μ₄)` with `q_*[u] = [χ]`. -/
def _root_.NCG.Multigraph.IsAmplitudeLift (u : H1Z4 G) (χ : H1 G) : Prop :=
  H1Z4.reduce G u = χ

open NCG.Multigraph in
/-- **Theorem `thm:finite-amplitude-lifts` (existence)**: every
orientation class admits a finite amplitude lift. -/
theorem _root_.NCG.Multigraph.amplitudeLift_exists (χ : H1 G) :
...
theorem _root_.NCG.Multigraph.amplitudeLift_card [Fintype G.V] [Fintype G.E] {v₀ : G.V}
...
def _root_.NCG.Multigraph.Walk.holonomy4 (u : G.E → ZMod 4) :
...
open NCG.Multigraph.Walk in
@[simp] theorem _root_.NCG.Multigraph.Walk.holonomy4_nil (u : G.E → ZMod 4) (v : G.V) :
    (Walk.nil v).holonomy4 u = 0 := rfl

open NCG.Multigraph.Walk in
@[simp] theorem _root_.NCG.Multigraph.Walk.holonomy4_fwd (u : G.E → ZMod 4) (e : G.E)
    {w : G.V} (p : G.Walk (G.tgt e) w) :
    (Walk.fwd e p).holonomy4 u = u e + p.holonomy4 u := rfl

open NCG.Multigraph.Walk in
@[simp] theorem _root_.NCG.Multigraph.Walk.holonomy4_bwd (u : G.E → ZMod 4) (e : G.E)
...
cochain. -/
theorem _root_.NCG.Multigraph.Walk.holonomy4_reduce (u : G.E → ZMod 4) :
...
theorem _root_.NCG.Multigraph.order_four_forced (u : G.E → ZMod 4) (χ : G.E → ZMod 2)
...
theorem _root_.NCG.Multigraph.no_canonical_z4_section (s : ZMod 2 →+ ZMod 4) :
...
def _root_.NCG.Multigraph.IsComplexAmplitudeRoot (z : G.E → ℂ) (lam : G.E → ℝˣ) : Prop :=
  ∀ e, (z e) ^ 2 = ((lam e : ℝ) : ℂ)

open NCG.Multigraph in
/-- The fourth root of unity attached to a `ℤ/4` phase exponent. -/
noncomputable def _root_.NCG.Multigraph.phaseOf (h : ZMod 4) : ℂ := Complex.I ^ h.val

open NCG.Multigraph in
theorem _root_.NCG.Multigraph.phaseOf_sq (h : ZMod 4) (χ : ZMod 2) (hred : z4ToZ2 h = χ) :
...
theorem _root_.NCG.Multigraph.amplitude_root_construction (lam : G.E → ℝˣ)
    (u : G.E → ZMod 4)
...
theorem _root_.NCG.Multigraph.amplitude_root_abs (z : G.E → ℂ) (lam : G.E → ℝˣ)
...
theorem _root_.NCG.Multigraph.amplitude_root_phase_sq (z : G.E → ℂ) (lam : G.E → ℝˣ)
    (hz : IsComplexAmplitudeRoot z lam) (e : G.E) :
    ((z e / (‖z e‖ : ℂ)) ^ 2
...
theorem _root_.NCG.Multigraph.amplitude_root_ratio (z z' : G.E → ℂ) (lam : G.E → ℝˣ)
...
theorem _root_.NCG.Multigraph.amplitude_root_mul_sign (z : G.E → ℂ) (lam : G.E → ℝˣ)
...
sector. -/
theorem _root_.NCG.Multigraph.amplitude_conservative (z z' : G.E → ℂ) (lam : G.E → ℝˣ)
    (hz : IsComplexAmplitudeRoot z lam)
    (hz' : IsComplexAmplitudeRoot z' lam) (e : G.E) :
