...
/-!
# Naimark, spectrum, and resolved-phase sharpness
...
proved `RenewalGeometry.locked_opportunity_contrast_spectrum`, re-exported by
...
seriality separation) are the proved `RenewalGeometry.resolved_phase_selection`,
...
`RenewalGeometry.finite_core_vs_growing_ledger`, re-exported by direct
...
`RenewalGeometry.frobenius_tensor_holonomy`, the bijectivity clause the proved
`RenewalGeometry.finite_tannaka_bijective` (both re-exported), and holonomy
...
`RenewalGeometry.coefficient_recovery`. -/
...
`RenewalGeometry.second_scalar_coupling`, re-exported by direct application). -/
...
`RenewalGeometry.representation_dimensions_not_duality`, re-exported by direct
application. -/
...
dimension at least `2ⁿ` (the proved `RenewalGeometry.boolean_record_dimension`,
...
`RenewalGeometry.parity_fold_iff`, re-exported) — so storage dimension alone
