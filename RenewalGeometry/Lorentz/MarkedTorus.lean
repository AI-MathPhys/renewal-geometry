...
`n = 0` (`RenewalGeometry.twisted_momentum_gap`,
`RenewalGeometry.twisted_momentum_gap_attained`).  Summing over directions, the
minimal twisted momentum squared is bounded below by
`π² · #{marked directions}` (`RenewalGeometry.marked_gap_sq`) — the spectral gap
