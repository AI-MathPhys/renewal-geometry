...
`sin(hξ)/h → ξ` (`RenewalGeometry.discrete_symbol_limit` — the slope of sine at
the origin).

**Theorem `thm:band-norm-resolvent`**: on a momentum band `|ξ| ≤ Λ`
the convergence is *uniform* with explicit cubic error:
`|sin(hξ)/h − ξ| ≤ h²Λ³/6` (`RenewalGeometry.band_symbol_bound`), from the global
sine inequality `|sin x − x| ≤ |x|³/6` (`RenewalGeometry.abs_sin_sub_le`).
