...
* **Lemma `lem:interference-lorentz-pair`** — the bracket relations of
  ...
  (`RenewalGeometry.bivector_vector_bracket`) and `[Kᵢ,Kⱼ] = ½ΓᵢΓⱼ = −Jᵢⱼ`
  (`RenewalGeometry.boost_bracket`) — the Cartan-decomposition relations of
...
* **Theorem `thm:interference-independence`** (dimension dichotomy):
  ...
  (`RenewalGeometry.closure_iso_of_three`); for `d ≥ 4` none exists
  (`RenewalGeometry.no_closure_iso`) — closure is satisfiable and falsifiable.
...
* **Theorem `thm:interference-closure-selects-three`** — `⋀²V = 0` at
  `d = 1` (`RenewalGeometry.wedge_two_dim_one`), and among `d ≥ 3` the volume-dual
  image has degree one iff `d = 3` (`RenewalGeometry.closure_selects_three`).

* **Definition `def:volume-dual-response`** — the volume element is
  invertible (`RenewalGeometry.volume_isUnit`, from `Vol² = ±1`), so
...
* **Propositions `prop:hodge-interference-closure`,
  ...
  (`RenewalGeometry.vol3_sq`), the `ε`-identities `ΓᵢΓⱼ = ε_{ijk}·Vol·Γₖ`
  (`RenewalGeometry.vol3_gamma01` …), the Hodge bracket is the cross product and
  satisfies Jacobi (`RenewalGeometry.hodge_bracket_jacobi`); the temporal
  anticommutation is `RenewalGeometry.temporal_anticomm_volume`. -/
