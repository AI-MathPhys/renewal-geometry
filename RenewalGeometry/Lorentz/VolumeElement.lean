...
* the **square identity** `Vol² = (−1)^{d(d−1)/2}·1`
  (`RenewalGeometry.volume_element_sq`), by the generator-exchange calculus of
  `NCG/Dimension/GradedGram.lean`;
* the temporal generator **anticommutes** with `Vol` for odd `d`
  (`RenewalGeometry.temporal_anticomm_volume` via `RenewalGeometry.gen0_mul_prod`), so `Vol` is
