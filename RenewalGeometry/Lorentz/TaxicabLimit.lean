...
(`RenewalGeometry.taxicab_reverse_triangle`), via the superadditivity of the
geometric mean — Mahler's inequality, proved here from two applications
of the weighted AM–GM (`RenewalGeometry.geomMean_superadditive`).  Together with
the order-monotone calibrated rescaling (`RenewalGeometry.calibratedRescale_mono`)
