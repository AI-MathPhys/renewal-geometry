...
* **Definition `def:kinematic-lorentzian`** — a kinematic Lorentzian
  ...
  reduction — encoded as `RenewalGeometry.KinematicLorentzianData`, with
  well-formedness `RenewalGeometry.KinematicLorentzianData.dirac_krein`.
...
* **Theorem `thm:assembly`** — the previously proved components
  ...
  (`RenewalGeometry.KinematicLorentzianData.mk` + `dirac_krein` +
  `twisted_reduction`).

* **Theorem `thm:regular-lorentzian`** — in the regular commuting
  regime the four pillars are the proved statements: `q_alg = c`
  (`RenewalGeometry.latticeShellCard_le`/`le_latticeShellCard`), interval growth
  `τ^c` (`RenewalGeometry.card_interval`), non-removability of `J_χ` for a nonzero
  class (`RenewalGeometry.Multigraph.nonremovable_of_class_ne_zero`), and the
  assembled signed Dirac.

* **Definition `def:rescaling`** — the calibrated rescaling
  `ι_R(x) = R⁻¹(ℓ₁x₁, …)` is monotone for the product orders
  (`RenewalGeometry.calibratedRescale_mono`), the order-theoretic core of the
...
`RenewalGeometry.symbol_through_second_moment`). -/
