...
* `RenewalGeometry.IsExtremeDir` — extreme directions of a convex cone;
* `RenewalGeometry.orthant` — the deterministic product-order cone `[0,∞)^c`;
* `RenewalGeometry.lorentzCone` — the round cone `{(t, v) : ‖v‖ ≤ t}`.

## Main results

* `RenewalGeometry.IsExtremeDir.map` — linear isomorphisms transport extreme
  directions;
* `RenewalGeometry.isExtremeDir_orthant_iff` — the orthant's extreme directions are
  exactly the positive multiples of the coordinate vectors
  (Corollary `cor:lorentz-violation`: the generator axes are canonical);
* `RenewalGeometry.isExtremeDir_lorentzCone` — every null direction of the round cone
  is extreme (uses strict convexity via `sameRay_iff_norm_add`);
* `RenewalGeometry.polyhedral_obstruction` — **Theorem `thm:obstruction`**: no linear
  equivalence maps the round Lorentz cone onto the orthant when the
  spatial space contains two independent directions;
* `RenewalGeometry.polyhedral_obstruction_complex` — the concrete `1+2`-dimensional
...
`thm:minkowski-2d` (`RenewalGeometry.minkowski_causal_order`) is consistent. -/
