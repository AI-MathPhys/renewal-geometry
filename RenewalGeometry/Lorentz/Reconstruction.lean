...
* **Theorem `thm:metric-reconstruction` /
  ...
  (`RenewalGeometry.quadratic_determines_symm`); recovering `M` from `M² = κ⁻²A`
...
* **Corollary `cor:31-endpoint`** (normalisation): with the
  ...
  `κ·(Mξ)ᵢ = ξᵢ` (`RenewalGeometry.endpoint_31_normalisation`) — with
  `RenewalGeometry.KreinCliffordDatum.multiplier_square` the explicit endpoint
...
* **Definition `def:strictly-marked-dirac-datum` /
  ...
  marked datum `(M, ρ)` is encoded (`RenewalGeometry.MarkedDiracDatum`), and the
  retained deck scalars `(−1)^{ρ_k}` determine the marked class
  (`RenewalGeometry.markedClass_injective`) — the classification separates marked
...
* **Theorem `thm:affine-modular`**: the eigenoperator relation
  ...
  `H = βN + c·1` (`RenewalGeometry.affine_modular`). -/
...
squares to `|ξ|²·1` (`RenewalGeometry.KreinCliffordDatum.multiplier_square`), the
free Dirac symbol. -/
