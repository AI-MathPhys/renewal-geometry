...
* **Theorem `thm:fundamental`** — `RenewalGeometry.cartan_uniqueness`: a difference
  of two metric torsion-free connections vanishes (the six-step index
  chase); `RenewalGeometry.koszul_antisym`, `RenewalGeometry.koszul_torsion`: the discrete
  Koszul formula `γ_{d;ae} = ½(C_{dea} − C_{ade} − C_{ead})` is metric
  and solves the torsion equation; `RenewalGeometry.cartan_derived_eq`: any metric
  torsion-free connection equals it.  (The `O(h²)` convergence to the
  Levi–Civita spin connection is not formalised.)

* **Proposition `prop:iso-metric`** — `RenewalGeometry.first_order_orthogonal`:
  `(1−hγ)ᵀ(1−hγ) = 1 − h(γ+γᵀ) + h²·γᵀγ`, so the transport is
  orthogonal to first order iff `γᵀ = −γ`
  (`RenewalGeometry.antisymm_iff_first_order_vanishes`,
  `RenewalGeometry.first_order_orthogonal_of_antisymm`).

* **Definition `def:plaquette-defect` / Lemma `lem:plaquette-torsion`**
  — `RenewalGeometry.plaquetteDefect`, `RenewalGeometry.plaquette_defect_eq`: the traversal
...
* **Definition `def:reset-motion` / Lemma `lem:plaquette-split`** —
  `RenewalGeometry.euclComp`, `RenewalGeometry.firstMoment`: reset channels as Euclidean
  motions `(R, t)`; `RenewalGeometry.translational_defect`: the first-moment
  plaquette defect is exactly `h²·T`; `RenewalGeometry.rotational_defect`: the
  rotation defect is exactly `−h²·F + h³·(cubic)`,
  `F = ∂_iγ_j − ∂_jγ_i + [γ_i, γ_j]`.

* **Theorem `thm:channel-torsion` / Corollary `cor:cix-discharged`** —
  `RenewalGeometry.coeff_extraction` + `RenewalGeometry.torsion_of_first_moment_closure`:
