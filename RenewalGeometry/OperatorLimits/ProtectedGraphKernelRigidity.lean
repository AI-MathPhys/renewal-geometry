...
theorem range_protected_eq_operatorGraphKernel_of_riesz_finrank_eq
    ...
    [Module.Finite ℂ
      (LinearMap.range
        (RenewalGeometry.ResolventStability.circleRieszProjection T center radius).toLinearMap)]
  ...
  have hkernelRiesz :
      operatorGraphKernel D A ≤
        LinearMap.range
          (RenewalGeometry.ResolventStability.circleRieszProjection T center radius).toLinearMap := by
