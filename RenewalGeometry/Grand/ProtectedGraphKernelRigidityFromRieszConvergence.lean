...
theorem eventually_range_protected_eq_operatorGraphKernel_of_riesz_tendsto
    ...
    (hfinite : ∀ᶠ n in atTop, Module.Finite ℂ
      (LinearMap.range
        (RenewalGeometry.ResolventStability.circleRieszProjection (T n) center radius).toLinearMap))
  ...
  have hRieszIdempotentCLM : ∀ᶠ n in atTop,
      IsIdempotentElem
        (RenewalGeometry.ResolventStability.circleRieszProjection (T n) center radius) :=
