...
namespace RenewalGeometry.PalatiniConnectionVariation
...
theorem palatini_connection_torsion_packet
  ...
  have htorsion : T = z :=
    RenewalGeometry.CartanInjectivity.spinless_torsion_zero K Pmap z hPinj hKinj T
      hCartanStationary
  obtain ⟨hinvLeft, hinvRight, hbound⟩ :=
    RenewalGeometry.palatini_torsion_core J hJ α β hab
  exact ⟨hcov, htorsion, hbound (V := V) (W := U) K T w κ hκ hfloor hKT,
    hinvLeft, hinvRight⟩

end RenewalGeometry.PalatiniConnectionVariation
