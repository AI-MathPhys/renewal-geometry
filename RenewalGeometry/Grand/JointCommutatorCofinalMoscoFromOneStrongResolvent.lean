...
theorem jointCommutatorEnergy_cofinalMoscoConverges_of_oneStrongResolvent
  ...
  apply J.ennrealBoundedOperatorEnergy_cofinalMoscoConverges_of_oneStrongResolvent_of_normalEquation
    (fun cutoff ↦ RenewalGeometry.jointCommutatorCLM (c cutoff)) A
    (RenewalGeometry.jointCommutatorResolventFamily c) T hdense lam0 hlam0 hT0
  · intro lam hlam cutoff f
    change RenewalGeometry.commutantLaplacianCLM (c cutoff)
        (RenewalGeometry.jointCommutatorResolventFamily c lam cutoff f) +
      (lam : ℂ) • RenewalGeometry.jointCommutatorResolventFamily c lam cutoff f = f
    simpa [RenewalGeometry.jointCommutatorResolventFamily,
      RenewalGeometry.jointCommutatorResolventAllShifts, dif_pos hlam] using
      RenewalGeometry.jointCommutatorResolvent_laplacianEquation
        (c cutoff) lam hlam f
  · exact hlimitNormal
