...
theorem jointCommutator_inverseNormGap_is_leastPositiveEigenvalue
    {n : Type u} [Fintype n] {s : ℕ} (c : Fin s → Matrix n n ℂ)
    (b : ℝ) (hb : 0 < b)
    (hne : RenewalGeometry.SpectralGap.complementCompression
  ...
  apply RenewalGeometry.SpectralGap.inverseNormGap_is_leastPositiveEigenvalue_of_normalResolvent
  ...
  · exact RenewalGeometry.VaryingHilbert.operatorGraphResolvent_isPositive
      (⊤ : Submodule ℂ (EuclideanSpace ℂ (n × n)))
      (RenewalGeometry.VaryingHilbert.boundedOperatorGraphMap (jointCommutatorCLM c)) b hb.le
