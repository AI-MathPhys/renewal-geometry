...
/-!
# Positive-semidefinite metric congruence and cocycle obstruction

Exact completion of `thm:renormalization-cocycle` for genuinely
positive-semidefinite, possibly singular metrics.  The main theorem proves:

* solvability of `Zᴴ B Z = A` exactly when `rank A ≤ rank B`;
* the complete pseudoinverse classification
  `Z = B^{†/2} U A^{1/2} + N`, with `U` an isometry between supports and
  `B N = 0`;
* the unitary support modulus when the ranks agree; and
* positivity and exact vanishing criteria for the cocycle diamond
  `Dᴴ G D`, including a concrete square-root failure witness.

Unlike the earlier positive-definite interface, all statements retain the
faithful support projections and therefore cover the singular quotient case
of the manuscript.
-/
...
theorem pinvSqrt_mul_mul {B : Matrix m m ℂ} (hB : B.PosSemidef) :
  ...
  · have hs : Real.sqrt (hB.1.eigenvalues i) ≠ 0 :=
    ...
    simp only [if_pos h]
    field_simp [id]
    simpa [id, pow_two] using (Real.mul_self_sqrt h.le).symm
...
theorem supportProj_mul_sqrtSF {B : Matrix m m ℂ} (hB : B.IsHermitian) :
  ...
  · rw [if_neg h, zero_mul,
      (Real.sqrt_eq_zero').2 (not_lt.mp h)]
  ...
  · intro hZ
    ...
    · -- `B Z (1 - supp A) = 0`, via a vanishing Gram
      ...
      have hBZ : B * (Z * ((1 : Matrix n n ℂ) - supportProj hA.1)) = 0 := by
        let T := Z * ((1 : Matrix n n ℂ) - supportProj hA.1)
        have hsqrt : sqrtSF hB.1 * sqrtSF hB.1 = B := sqrtSF_mul_self hB
        calc B * T = (sqrtSF hB.1 * sqrtSF hB.1) * T := by rw [hsqrt]
          _ = sqrtSF hB.1 * (sqrtSF hB.1 * T) := Matrix.mul_assoc _ _ _
...
theorem stiefelPattern_cogram (hA : A.IsHermitian) (hB : B.IsHermitian)
  ...
  · rw [Finset.sum_congr rfl fun i _ => by
      ...
      · rw [dif_pos hi, if_neg (fun hc : j = (e ⟨i, hi⟩ : _).1 =>
          hj (hc ▸ (e ⟨i, hi⟩).2 : hB.eigenvalues j ≠ 0)), zero_mul]
      · rw [dif_neg hi, zero_mul]]
    rw [Finset.sum_const_zero]
    by_cases hjj : j = j'
    · subst hjj
...
theorem stiefelIsometry_gram (hA : A.PosSemidef) (hB : B.PosSemidef)
  ...
  have hUB : (hB.1.eigenvectorUnitary : Matrix m m ℂ)ᴴ
    ...
    UnitaryGroup.star_mul_self hB.1.eigenvectorUnitary
...
theorem renormalization_cocycle_exact {n m : Type*} [Fintype n] [Fintype m]
  ...
  · intro hrank
    ...
    have hUA : (hA.1.eigenvectorUnitary : Matrix n n ℂ)ᴴ
      ...
      UnitaryGroup.star_mul_self hA.1.eigenvectorUnitary
      ...
      · intro h0 v
        have hSD : sqrtSF hG.1 * D = 0 := by
          ...
          have hfac : (sqrtSF hG.1 * D)ᴴ * (sqrtSF hG.1 * D) = 0 := by
            rw [Matrix.conjTranspose_mul, (sqrtSF_isHermitian hG.1).eq]
            have hsqrt : sqrtSF hG.1 * sqrtSF hG.1 = G := sqrtSF_mul_self hG
            calc Dᴴ * sqrtSF hG.1 * (sqrtSF hG.1 * D)
                = Dᴴ * ((sqrtSF hG.1 * sqrtSF hG.1) * D) := by
                    simp only [Matrix.mul_assoc]
              _ = Dᴴ * (G * D) := by rw [hsqrt]
              _ = 0 := by rw [hGD0, Matrix.mul_zero]
          exact Matrix.conjTranspose_mul_self_eq_zero.mp hfac
        have hv := congrArg (fun M : Matrix m m ℂ => M *ᵥ v) hSD
