...
namespace RenewalGeometry.CompactNormalFiniteSpectralScreensExact
...
theorem spectralScreenSubspace_adjoint_invariant
  ...
  have hmap : Submodule.map (ContinuousLinearMap.adjoint R).toLinearMap
    ...
    · intro μ hμ y hy
      ...
      rw [RenewalGeometry.NormalSpectrum.adjoint_apply_eigenvector_of_isStarNormal
        R hnormal hzEig]
...
theorem tendsto_spectralScreen_apply
    ...
    Tendsto (fun s : Finset ℂ => spectralScreen R hcompact hinjective s x)
      atTop (𝓝 x) := by
  have hdenseEigen :=
    RenewalGeometry.NormalSpectrum.dense_iSup_eigenspaces_of_compact_of_isStarNormal
      R hcompact hnormal
...
end RenewalGeometry.CompactNormalFiniteSpectralScreensExact
