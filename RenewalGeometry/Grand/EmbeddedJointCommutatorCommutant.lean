...
theorem embeddedMatrix_mem_mappedJointCommutatorKernel_iff
  ...
  constructor
  · rintro ⟨y, hy, hembed⟩
    have hyX : y = RenewalGeometry.matrixL2 X :=
      (J.embedding cutoff).injective hembed
    apply (RenewalGeometry.matrixL2_mem_jointCommutatorCLM_ker_iff (c cutoff) X).mp
    simpa [hyX] using hy
  · intro hcomm
    refine ⟨RenewalGeometry.matrixL2 X,
      (RenewalGeometry.matrixL2_mem_jointCommutatorCLM_ker_iff (c cutoff) X).mpr hcomm, rfl⟩
