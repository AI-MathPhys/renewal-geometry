...
namespace RenewalGeometry.ResolventStability
...
theorem circleRieszProjection_tendsto_of_collectivelyCompactStrong
    ...
    (hcompact :
      (RenewalGeometry.VaryingHilbert.constantSystem ℂ H).CollectivelyCompact T)
...
theorem compactScreen_spectralConsequences
    ...
    IsCompactOperator Tlim ∧
      ...
      Tendsto
        ...
        (𝓝 (RenewalGeometry.SpectralApproximation.sourceGram
  ...
  · exact RenewalGeometry.ProjectionStability.eventually_finrank_range_eq_of_tendsto
      (fun n ↦ circleRieszProjection (T n) center radius)
      (circleRieszProjection Tlim center radius)
      hproj hidem_seq hidem hfinite_seq
  · exact RenewalGeometry.SpectralApproximation.sourceGram_tendsto hproj hv
...
theorem compactScreen_spectralConsequences_of_zero_avoiding
  ...
  have hstageCompact : ∀ n, IsCompactOperator (T n : H → H) := by
    intro n
    simpa [RenewalGeometry.VaryingHilbert.System.embeddedOperator,
      RenewalGeometry.VaryingHilbert.constantSystem] using
        RenewalGeometry.VaryingHilbert.System.CollectivelyCompact.isCompactOperator_embedded
          (RenewalGeometry.VaryingHilbert.constantSystem ℂ H) hcompact n
...
theorem compactScreen_spectralConsequences_of_zero_avoiding_of_endpoints
  ...
  have hop : Tendsto T atTop (𝓝 Tlim) :=
    RenewalGeometry.VaryingHilbert.System.tendsto_operatorNorm_of_collectivelyCompact_of_symmetric'
...
end RenewalGeometry.ResolventStability
