...
open RenewalGeometry.ResolventStability
...
theorem jointCommutator_protectedKernelRigidity_of_denseSources_of_graphScreens
    ...
    (hprotected : ∀ᶠ cutoff in atTop,
      LinearMap.range (P cutoff).toLinearMap ≤
        (LinearMap.ker
          (RenewalGeometry.jointCommutatorCLM (c cutoff)).toLinearMap).map
    ...
    ∃ radius : ℝ, 0 < radius ∧
      ...
      Tendsto
        (fun cutoff ↦ RenewalGeometry.ResolventStability.circleRieszProjection
