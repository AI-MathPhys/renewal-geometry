...
theorem circleRieszProjection_ne_one_of_not_finiteDimensional
    ...
    RenewalGeometry.ResolventStability.circleRieszProjection T center radius ≠ 1 := by
  intro hQ
  have hQcompact : IsCompactOperator
      ((RenewalGeometry.ResolventStability.circleRieszProjection T center radius : E →L[ℂ] E) : E → E) :=
...
theorem circleRieszProjection_complementCompression_ne_zero_of_injective
    ...
    complementCompression T
      (RenewalGeometry.ResolventStability.circleRieszProjection T center radius) ≠ 0 := by
  let Q := RenewalGeometry.ResolventStability.circleRieszProjection T center radius
...
theorem norm_complementCompression_circleRieszProjection_lt
    ...
    (hnorm : ‖T‖ ≤ mu)
    (hne : complementCompression T (RenewalGeometry.ResolventStability.circleRieszProjection
      T center radius) ≠ 0) :
    ‖complementCompression T (RenewalGeometry.ResolventStability.circleRieszProjection
      T center radius)‖ < mu := by
  let Q : E →L[ℂ] E :=
    RenewalGeometry.ResolventStability.circleRieszProjection T center radius
  ...
  have hTx : T x = (mu : ℂ) • x := by
    calc
      ...
      _ = (mu : ℂ) • x := by rw [hCeq]
  have hfix : Q x = x :=
    RenewalGeometry.ResolventStability.circleRieszProjection_apply_eigenvector_of_mem_ball
...
theorem inverseNormGap_circleRieszProjection_pos
    ...
    (hne : complementCompression T (RenewalGeometry.ResolventStability.circleRieszProjection
      T center radius) ≠ 0) :
    0 < ‖complementCompression T (RenewalGeometry.ResolventStability.circleRieszProjection
