...
theorem jointCommutatorResolvent_strongOperatorConverges_of_denseSources
    ...
    (hconv : ∀ x ∈ D, J.StronglyConverges
      (fun cutoff ↦ RenewalGeometry.jointCommutatorResolventFamily c lam cutoff
        (source x cutoff))
      (T x)) :
    J.StrongOperatorConverges J
      (RenewalGeometry.jointCommutatorResolventFamily c lam) T := by
  apply J.strongOperatorConverges_of_dense_sources_of_uniform_opNorm
    J (RenewalGeometry.jointCommutatorResolventFamily c lam) T
  ...
  · intro cutoff
    exact operatorGraphResolvent_opNorm_le_inv
      ...
      (RenewalGeometry.jointCommutatorResolventFamily c lam cutoff) lam hlam
...
theorem jointCommutatorResolvent_cofinalStrongOperatorConverges_of_denseSources
    ...
    ∀ (φ : ℕ → ℕ), Tendsto φ atTop atTop →
      (J.reindex φ).StrongOperatorConverges (J.reindex φ)
        (fun cutoff ↦
          RenewalGeometry.jointCommutatorResolventFamily c lam (φ cutoff)) T := by
  intro φ hφ
  apply (J.reindex φ).strongOperatorConverges_of_dense_sources_of_uniform_opNorm
    (J.reindex φ)
    (fun cutoff ↦ RenewalGeometry.jointCommutatorResolventFamily c lam (φ cutoff))
  ...
  · intro cutoff
    exact operatorGraphResolvent_opNorm_le_inv
      ...
      (boundedOperatorGraphMap (RenewalGeometry.jointCommutatorCLM (c (φ cutoff))))
      (RenewalGeometry.jointCommutatorResolventFamily c lam (φ cutoff)) lam hlam
      (RenewalGeometry.jointCommutatorResolventFamily_resolventEquation
        c lam hlam (φ cutoff))
  · intro x hx
