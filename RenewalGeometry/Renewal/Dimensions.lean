...
* **Definition `def:qalg`** (algebraic predictive dimension): the
  ...
  (`RenewalGeometry.RenewalMemory.algebraicDimension`).  Its well-posedness is the
  **finite-shell theorem** (`RenewalGeometry.RenewalMemory.shell_finite`, Remark
...
* **Definition `def:qmet`** (metric predictive dimension): the upper
  ...
  (`RenewalGeometry.coveringNumber`, `RenewalGeometry.upperBoxDimension`).  The completely
