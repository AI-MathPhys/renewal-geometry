...
`M := NCG.UCPMap A` (see `RenewalGeometry.CPRenewalMemory`).  All of the predictive
...
* `RenewalGeometry.RenewalMemory E M` — a renewal memory with channels in `M`;
* `RenewalGeometry.RenewalMemory.History` — histories as elements of `FreeMonoid E`;
* `RenewalGeometry.RenewalMemory.acc` — the accumulated-channel monoid homomorphism
  `FreeMonoid E →* M`;
* `RenewalGeometry.CPRenewalMemory E A` — the completely positive realization.
-/
