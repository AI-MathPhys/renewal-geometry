...
`(R+1)^c ≤ c! · N_c(R) ≤ (R+c)^c` (`RenewalGeometry.weyl_free_bounds`),
...
(`RenewalGeometry.renewal_pole_identity`) whose second factor evaluates at `z = 1`
to the mean return length `μ = Σ n·fₙ` (`RenewalGeometry.renewal_mean_at_one`) —
...
(`RenewalGeometry.gamma_reflection_renewal`) which converts the Tauberian constant
