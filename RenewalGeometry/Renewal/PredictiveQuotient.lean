...
attained (`RenewalGeometry.RenewalMemory.exists_quotLength_rep`).

## Main definitions

* `RenewalGeometry.RenewalMemory.predCon` — the predictive congruence `∼_CP`;
* `RenewalGeometry.RenewalMemory.PredictiveQuotient` — the quotient `𝒲_CP`, a monoid;
* `RenewalGeometry.RenewalMemory.mkQ` — the projection onto predictive classes;
* `RenewalGeometry.RenewalMemory.shift` — the descended shift `Ŝ_σ [w] = [σ w]`;
* `RenewalGeometry.RenewalMemory.quotLength` — the quotient length `Λ_min`.

## Main results

* `RenewalGeometry.RenewalMemory.shift_congruence` — Lemma `lem:shift-congruence`;
* `RenewalGeometry.RenewalMemory.quotientEquivRange` — the predictive quotient is
  isomorphic to the accumulated-channel monoid `ℳ ⊆ M`;
* `RenewalGeometry.RenewalMemory.one_le_quotLength`,
  `RenewalGeometry.RenewalMemory.quotLength_mkQ_le`,
  `RenewalGeometry.RenewalMemory.exists_quotLength_rep` — basic properties of `Λ_min`.
-/

namespace RenewalGeometry.RenewalMemory
...
end RenewalGeometry.RenewalMemory
