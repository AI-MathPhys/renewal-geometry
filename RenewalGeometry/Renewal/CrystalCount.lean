...
`RenewalGeometry.latticeBallCard_eq`, `RenewalGeometry.crystalCount_card`), and its log–log
dimension is **exactly** `1 + b_eff` as an actual limit
(`RenewalGeometry.crystal_log_dimension`) — the link `q_alg = 1 + b_eff` of the
