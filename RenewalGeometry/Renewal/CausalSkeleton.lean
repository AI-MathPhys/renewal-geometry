...
* **local finiteness** (`RenewalGeometry.CausalSkeleton.interval_finite`): every
  order interval `[x, y]` embeds in the finite length ball of radius
  `T(y)`, hence is finite;
* **gradedness** (`RenewalGeometry.CausalSkeleton.saturated_chain_rank`,
  `RenewalGeometry.CausalSkeleton.saturated_chain_length_eq`): along any saturated
...
function `T : P → ℕ`; the predictive poset `RenewalGeometry.PredictivePoset` of
...
namespace RenewalGeometry.CausalSkeleton
...
end RenewalGeometry.CausalSkeleton
