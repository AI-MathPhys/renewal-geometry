...
* **Proposition `prop:fibres-automata`** (graded finite-type automata):
  ...
  (`RenewalGeometry.RenewalMemory.quotLength_shift_le`); with the graded-automaton
...
* **Proposition `prop:fibres-cp`** (commuting CP channels): a
  ...
  (`RenewalGeometry.RenewalMemory.shift_injective_of_cancel` — the finite
...
* **Proposition `prop:fibres-quotient`** (bounded rank distortion): a
  ...
  `≤ C + 2D` (`RenewalGeometry.rank_distortion_increment`); the fibre-count
...
`RenewalGeometry.RenewalMemory.shift_injective` applies. -/
