...
coordinate (`RenewalGeometry.shellSigmaEquiv`), apply the inductive hypothesis to
each fibre, and close with the hockey-stick identity
`∑_{m ∈ [k, n]} C(m, k) = C(n + 1, k + 1)`.

The shell increment `N(R+1) − N(R) = C(R + c, c − 1)`
(`RenewalGeometry.latticeShellCard_shell`) is the exact degree multiplicity used by
