% Integrated revision: twentieth-cycle V72 plus perturbative ESM continuations through bosonic V80, general V44, and phenomenological V40.
% ==================== 00_front.tex ====================
...
Canonical fermion normalization and fixed-parent phase space then distinguish quantum pair conversion from the commuting triad amplitudes.  A projected one-parent Hamiltonian admits a subtraction-defined self-adjoint ultraviolet limit, an explicit spectral measure, threshold tails, and a weak-coupling exponential survival limit.  Common-profile parent channels yield coherent decay and a precisely invariant dark subspace.  At finite density, exact exclusion-leakage and weighted-correlation identities obstruct naive bosonization and entrywise decorrelation.  An exclusion-preserving collective model instead has a global ultraviolet mild flow and a conserved dressed energy.  Restoring all retained momentum pairings gives a matrix-Pauli covariance flow and a fixed-time many-copy transport theorem for empirical polynomial moments.  These are constructions in explicitly specified models and limit orders, not a finite-species quantum kinetic limit or a realization of the complete Standard Model--Einstein continuum.  Likewise, the perturbative shell results are coefficientwise finite-order statements rather than an analytic finite-coupling RG trajectory: the two-loop bosonic closure does not yet include chiral fermions or Yukawa couplings, and the Euclidean exceptional-factorization theorem does not supply physical Minkowski pinch-surface or inclusive soft--collinear control.  Differentiated proper-time trace-class transport and a quantitative three-defect estimate sharpen the determinant-to-stress and constraint-to-Einstein interfaces of the conditional chain.
...
existence in the first sense need not establish existence in the second.  A
\emph{coefficientwise perturbative continuum}---cutoff removal at every
prescribed finite loop order, EFT weight, source arity, and derivative order---
is likewise distinct from an analytic finite-coupling RG trajectory, a
convergent all-orders expansion, or a nonperturbative probability law.
...
\subsection{Finite-cell cohomology and thermodynamic displacement}
A finite-state coboundary and an unbounded thermodynamic displacement obey the
same algebraic telescoping identity but need not have the same transport
limit.  This distinction is relevant whenever the metric is calibrated from a
momentum tilt or Green--Kubo covariance rather than from a bounded one-cell
observable.

\begin{proposition}[Finite coboundary versus thermodynamic transport]
\label{prop:finite-thermodynamic-coboundary}
Let $P$ be an irreducible stochastic matrix on a finite state set and let
$\xi(x,y)=\phi(y)-\phi(x)$.  For the tilted matrix
\begin{equation}
 (P_t)_{xy}=P_{xy}e^{t\xi(x,y)},\qquad
 D_t=\diag(e^{t\phi(x)}),
\end{equation}
one has $P_t=D_t^{-1}PD_t$.  Hence its Perron pressure is identically
zero and every extensive Perron cumulant of $\xi$ vanishes.  This conclusion
need not survive a thermodynamic embedding in which $\phi$ becomes unbounded.
For the simple random walk on $\mathbb Z$ with $\phi(x)=x$,
\begin{equation}
 \lim_{n\to\infty}\frac1n\log\E_0
 e^{t(\phi(X_n)-\phi(X_0))}=\log\cosh t,
 \qquad \Lambda''(0)=1,
 \label{eq:thermodynamic-coboundary-example}
\end{equation}
whereas every fixed reflecting interval has zero Perron pressure for the same
state coboundary.
\end{proposition}
\begin{proof}
On a fixed finite carrier, diagonal similarity preserves the complete spectrum
and the path sum telescopes to the bounded endpoint difference
$\phi(X_n)-\phi(X_0)$.  On $\mathbb Z$, the increments are independent
$\pm1$ variables, so their moment generating function is $(\cosh t)^n$.
The conjugating multiplier $e^{t\phi}$ is then unbounded for real $t\ne0$ on
the natural unweighted infinite-volume spaces.  Thus the fixed-volume
long-time limit and the thermodynamic/pre-boundary transport limit do not
commute in general.
\end{proof}
\src{PP40}{thm:v40-fixed-finite-coboundary,prop:v40-z-counterexample}

The appropriate finite-volume approximation to the second regime is a
pre-boundary window.  If $\tau_R$ is the exit time from a ball $B_R$, a coupling
with the infinite walk gives, for every Fourier momentum $k$,
\begin{equation}
 \left|\E e^{\ii k(\phi(X_n)-\phi(X_0))}
 -\E\!\left[e^{\ii k(\phi(X_n)-\phi(X_0))}\one_{\{\tau_R>n\}}\right]\right|
 \le \mathbb P(\tau_R\le n).
 \label{eq:preboundary-transport}
\end{equation}
Consequently a scale choice $n=n(R)\to\infty$ with
$\mathbb P(\tau_R\le n(R))\to0$ recovers thermodynamic transport before the
fixed-box equilibrium limit annihilates bounded coboundaries.

There is a second finite-cell distinction.  If
$U\simeq\mathbf1\oplus\mathbf3$ is the vertex permutation representation of
$K_4$, the unsigned six-edge amplitudes and oriented antisymmetric one-cochains
carry different $S_4$ modules:
\begin{equation}
 E^+\simeq\mathbf1\oplus\mathbf2\oplus\mathbf3,
 \qquad
 E^-\simeq\Lambda^2U\simeq\mathbf3\oplus\mathbf3',
 \qquad
 H^1_{\rm or}(K_4;\R)\simeq\mathbf3'.
 \label{eq:k4-edge-representation-fork}
\end{equation}
The last quotient follows because the graph coboundary maps the reduced vertex
$\mathbf3$ isomorphically onto the first $\mathbf3$ summand of $E^-$.  Thus an
unsigned bond amplitude and a reversal-odd displacement cochain cannot be
identified merely because both lead to three-dimensional quotients.
\src{PP40}{thm:v40-representation-fork}

For an operational event record the physical displacement is therefore kept in
the incidence-resolved form
\begin{equation}
 \xi_{\rm phys}
 =A_{\rm cell}\xi_{\rm cell}^{\rm fin}
 +A_{\rm skel}\xi_{\rm skel}^{\rm th}
 +A_{\rm ext}\xi_{\rm ext}^{\rm MA}.
 \label{eq:three-layer-displacement}
\end{equation}
Only the bounded finite-cell harmonic term is classified by $K_4$ periods.  An
unbounded skeleton gradient is classified by its thermodynamic transport
packet, and a genuinely Markov-additive external mark has its own covering or
port data.  The load-bearing requirement is that the same-record writer in
\eqref{eq:three-layer-displacement} reproduce the already calibrated metric
covariance.  In particular, the internal cycle fibre used for generation
counting is not promoted to a spatial displacement space without a measured
incidence map.  \src{PP40}{def:v40-two-scale-certificate}

\scope{This separation strengthens the geometric compiler without selecting
which displacement block is physically populated.  The incidence maps
$A_{\rm cell},A_{\rm skel},A_{\rm ext}$ and the same-writer Green--Kubo match
remain empirical/constructive inputs of the chosen renewal family.}
...
\subsection{Pure-regulator topology on a protected overlap component}
The determinant-line problem can be sharpened by separating topology carried
by the physical gauge/geometric base from topology introduced only by a choice
of microscopic regulator.  On a uniformly protected minimal overlap component,
the latter is contractible in the native exponentially local category.

\begin{theorem}[Protected pure-regulator contraction]
\label{thm:pure-regulator-contraction}
Let $B$ be a compact physical parameter panel and $Y$ a compact regulator
parameter space.  For all sufficiently small lattice scales $a$, suppose the
native overlap projectors $P_a(b,y)$ lie in one fixed-index minimal component,
have the stated uniform reduced-gap and source/BV derivative margins, and are
exponentially local and gauge/spin covariant.  Fix a reference regulator
$y_*\in Y$.  Then there is a source-complete native homotopy, relative to
$B\times\{y_*\}$,
\begin{equation}
 P_{a,s}:B\times Y\longrightarrow\mathcal M_{k,a}^{\rm prot},
 \qquad
 P_{a,0}(b,y)=P_a(b,y),\quad
 P_{a,1}(b,y)=P_a(b,y_*).
 \label{eq:pure-regulator-contraction}
\end{equation}
Equivalently,
\begin{equation}
 P_a\simeq_{\rm nat}\pi_B^*P_{a,*}.
 \label{eq:pure-regulator-pullback}
\end{equation}
Hence every characteristic or clutching class supported purely on $Y$ vanishes.
After the separately identified local/quasilocal curvature of the determinant
connection is removed, the residual flat pure-regulator holonomy character is
also trivial.
\end{theorem}
\begin{proof}[Proof mechanism]
Uniform protection first aligns the forced kernel/cokernel plane with its
reference plane by a controlled direct rotation.  With that plane fixed, the
minimal projector is a graph over the reference subspace; linear contraction
of the graph coordinate stays in the same minimal component and preserves the
uniform reduced gap.  Functional calculus transports exponential locality and
the retained source derivatives along the homotopy.  Sign flattening
$H\mapsto2P-1$ lifts the contraction to the native Hermitian-kernel category.
Every pure-regulator loop therefore admits a native disk filling, which kills
the residual flat character after the local curvature primitive has been
separated.
\end{proof}
\src{PG44}{thm:v42-full-contraction,cor:v42-no-higher-projector,thm:v42-flat-trivial}

\scope{The theorem removes a regulator-only topological obstruction inside the
declared protected component.  It does not trivialize topology carried by the
physical gauge/geometric base, set the raw Berry curvature or raw Berry
holonomy to zero, orient a real/Pfaffian line, prove reflection positivity, or
permit a path through a Wilson wall or a physical zero-mode crossing.}
...
\section{A coefficientwise bosonic Einstein--gauge--Higgs two-loop branch}
\label{sec:perturbative-egh}
The abstract shell architecture has a nontrivial finite-order realization in
which gravity, all three Standard Model gauge factors, and the charged Higgs
are carried by one retained-constraint BV/source state.  This branch is
coefficientwise: loop order, local weight, source arity, and the perturbative
coupling domain are fixed before the ultraviolet shell limit is taken.

Let $\mathsf P^{\rm EGH}_{N,6}$ be the complete local Taylor/tensor projector at
source arity $N$ and local weight six, and write
\begin{equation}
 d^{\rm EGH}_{N,6}
 =\mathsf P^{\rm EGH}_{N,6}
   \bigl((S_0^{\rm EGH},\,\cdot\,)\bigr)
   \mathsf P^{\rm EGH}_{N,6}.
 \label{eq:egh-bv-differential}
\end{equation}
The retained bosonic fields are the gravitational/nonminimal packet,
$SU(3)$, $SU(2)$ and $U(1)_Y$ gauge/ghost fields, and the charged Higgs
$H\in\C^2$.  Chiral fermions and Yukawa couplings are not included in this
subsection.

\begin{theorem}[Bosonic EGH two-loop filtered self-map]
\label{thm:bosonic-egh-two-loop}
Fix perturbative order two, local weight six, source arity $N$, the declared
profile and hard-inverse margins, and one common set of lower-weight physical
normalization conditions.  Retain the soft gravitational constraints rather
than eliminating them through a nonlocal inverse.  Then:
\begin{enumerate}[label=\textup{(B\arabic*)},leftmargin=3em]
\item the projected local complex is finite dimensional and
$(d^{\rm EGH}_{N,6})^2=0$;
\item because the Higgs has nonzero hypercharge, $U(1)_Y$ is not a free
abelian factor acting trivially on matter, so the special free-abelian
antifield-dependent anomaly family is absent; the remaining ghost-one quotient
is the ordinary finite consistent-gauge-anomaly space;
\item the complete native bosonic hard-shell breaking has zero projection onto
that ordinary anomaly quotient.  Consequently the populated weight-six
local two-loop row lies in the BV range and admits the minimum-norm restoring
functional
\begin{equation}
 C_{2,j}^{\rm rest,bos}
 =-\bigl(d^{\rm EGH}_{N,6}\bigr)^\dagger B^{\rm EGH}_{2,j},
 \qquad
 \norm{C_{2,j}^{\rm rest,bos}}
 \le (\sigma^{\rm EGH}_{N,6})^{-1}\norm{B^{\rm EGH}_{2,j}};
 \label{eq:egh-restorer}
\end{equation}
\item the independent ghost-zero weight-six EFT quotient is not removed by
this restoration.  It contains representatives of the schematic forms
$R\tr F^2$, $(H^\dagger H)\tr F^2$, $R|DH|^2$, $|DH|^4$ and
$(H^\dagger H)^3$, together with their curvature, integration-by-parts,
equation-of-motion and basis-completion partners;
\item after every proper one-loop subgraph is subtracted by the same common
one-loop action and the complete weight-six local Taylor assignment is
removed, each two-loop hard-annulus graph satisfies
\begin{equation}
 |\partial_K^\alpha R^{\rm EGH}_{G,j}(K)|
 \le C_{G,\alpha}\Lambda_j^{-1}
       |K|^{\omega_G+1-|\alpha|},
 \qquad
 \norm{(1-\mathsf P^{\rm EGH}_{N,6})
       \mathcal K^{\rm EGH}_{2,j}}_{*\to *}
 \le C_{N,2}\frac{\mu}{\Lambda_j};
 \label{eq:egh-hard-annulus}
\end{equation}
\item the resulting normalized measured shell defines a two-loop filtered
self-map of the complete bosonic EGH state, including its physical Wilson
coordinates and source/BV packet.
\end{enumerate}
\end{theorem}
\begin{proof}[Proof mechanism]
The classical coupled BV action gives the nilpotent unprojected differential;
the complete weight-six descendant bank makes the local projection a chain
map.  Nonzero Higgs hypercharge excludes the free-abelian matter-trivial
branch.  On each finite hard carrier, bosonic and ghost Hessians transform by
similarity, so cyclicity of the finite trace gives zero gauge variation of
their log determinants.  Haar measure and the real Higgs coefficient measure
are invariant as well.  Thus the native bosonic shell has zero coordinate in
the remaining anomaly quotient, and finite-dimensional Hodge decomposition
produces \eqref{eq:egh-restorer}.  Symmetry restoration acts only in ghost
number one, leaving the ghost-zero Wilson quotient independent.  Finally the
common forest subtraction leaves one additional external-momentum power in the
hard-annulus complement; scale ordering gives the $\Lambda_j^{-1}$ factor in
\eqref{eq:egh-hard-annulus}.  Retention of the soft constraints prevents an
inverse soft wave operator from spoiling this local counting.
\end{proof}
\src{P80}{thm:v80-complex,thm:v80-native-zero,thm:v80-range,thm:v80-annulus,thm:v80-selfmap}

If
$\mathsf U^{\rm EGH,[\le2]}_{j+1,j}
 =I+E^{\rm EGH}_{1,j}+E^{\rm EGH}_{2,j}$
is the adjacent source map, the same hard-annulus estimate gives
\begin{equation}
 \norm{E^{\rm EGH}_{1,j}}_{*\to *}
 +\norm{E^{\rm EGH}_{2,j}}_{*\to *}
 \le C\frac{\mu}{\Lambda_j},
 \label{eq:egh-source-step}
\end{equation}
with the quadratic one-loop connection term bounded by
$C(\mu/\Lambda_j)^2$.  For geometric shell ratio $b>1$ this yields
\begin{equation}
 \sup_{n\ge j}\norm{\mathsf U^{\rm EGH,[\le2]}_{n,j}}_{*\to *}
 \le
 \exp\!\left(\frac{Cb}{b-1}\frac{\mu}{\Lambda_j}\right).
 \label{eq:egh-composed-source}
\end{equation}
Thus source transport is uniform in the number of ultraviolet shell steps at
this fixed perturbative/EFT order.  \src{P80}{cor:v80-source-product}

\scope{This is a genuine coefficientwise two-loop closure theorem, not the
nonperturbative endpoint of Chapter~\ref{ch:closure}.  It supplies neither an
all-orders convergent trajectory nor the chiral fermion/Yukawa determinant
line.  Once chiral matter is inserted, Standard Model anomaly cancellation
must be established for the assembled fermionic representation and tied to the
same determinant/Pfaffian transport.  Numerical beta functions and finite
Wilson matching coefficients are also separate data.}
...
\section{Coefficientwise massless Euclidean completion}
\label{sec:perturbative-ir}
For the perturbative branch, the infrared ledger can be closed at every fixed
finite coefficient order on a precisely stated Euclidean observable class.
The renormalization point and the auxiliary infrared floor must first be
separated: a massless renormalized theory may keep a nonzero subtraction point
$\mu_R$ while the soft floor $\rho$ is removed.

For external and source momenta $\mathbf q=(q_1,\ldots,q_n)$ with
$\sum_iq_i=0$, define the nonexceptionality modulus
\begin{equation}
 \eta(\mathbf q)=
 \min_{\varnothing\ne S\subsetneq\{1,\ldots,n\}}
 \left|\sum_{i\in S}q_i\right|.
 \label{eq:nonexceptionality-modulus}
\end{equation}
A compact Euclidean panel is strictly nonexceptional when
$\eta(\mathbf q)\ge\kappa>0$ and all external/source momenta are bounded by a
fixed $Q$.

\begin{theorem}[Nonexceptional zero-floor perturbative continuum]
\label{thm:nonexceptional-zero-floor}
Fix finite loop/EFT/source data $(L,\Delta,r,N)$, one coherent positive
renormalization point $\mu_R$, the protected native regulator branch, and a
compact strictly nonexceptional Euclidean panel.  After the ultraviolet forest
subtractions, every retained normalized connected coefficient and every
retained momentum/source derivative has a unique zero-floor limit
\begin{equation}
 W_{\infty,0}(\mathbf q)
 =\lim_{\rho\downarrow0}W_{\infty,\rho}(\mathbf q)
 \label{eq:zero-floor-limit}
\end{equation}
uniformly on the panel.  There are finite packet-dependent
$\sigma_*>0,M_*<\infty$ such that
\begin{equation}
 \norm{W_{\infty,\rho}-W_{\infty,0}}
 \le C\left(\frac{\rho}{\kappa}\right)^{\sigma_*}
 \left[1+\left|\log\frac{\rho}{\kappa}\right|^{M_*}\right].
 \label{eq:zero-floor-rate}
\end{equation}
For a finite ultraviolet top shell $\Lambda_J$ the two tails can be combined as
\begin{equation}
 \norm{W_{J,\rho}-W_{\infty,0}}
 \le C_{\rm UV}\frac{Q_R}{\Lambda_J}
 +C_{\rm IR}\left(\frac{\rho}{\kappa}\right)^{\sigma_*}
 \left[1+\left|\log\frac{\rho}{\kappa}\right|^{M_*}\right],
 \label{eq:uv-ir-two-tail}
\end{equation}
so ultraviolet removal and zero-floor completion commute coefficientwise on
this panel.  The common finite BV/BRST/master/source identities, exact
loop-order projectivity, and finite changes of positive renormalization point
survive the limit.  No new infrared counterterm is introduced.
\end{theorem}
\begin{proof}[Proof mechanism]
After the ultraviolet forests are prepared, every graph/source descendant has
only finitely many Euclidean soft sectors at fixed $(L,\Delta,r,N)$.  In a
strictly nonexceptional panel, a propagator carrying a nonzero partial external
transfer remains uniformly hard when the remaining loop variables are soft.
A genuine soft radial variable therefore appears with
$t^{\sigma-1}(1+|\log t|)^M$ for some $\sigma>0$; the finite packet has a common
minimum $\sigma_*>0$.  Geometric soft-shell decomposition then produces a
summable tail.  The algebraic identities are finite polynomial relations among
the retained coefficients and therefore pass through uniform coefficientwise
convergence.  The independent ultraviolet and infrared tails give
\eqref{eq:uv-ir-two-tail}.
\end{proof}
\src{PG44}{thm:v43-ir-one-excess,thm:v43-zero-floor,thm:v43-uv-ir-commute,thm:v43-master,thm:v43-projective}

The loss of a positive nonexceptionality margin does not require declaring the
Euclidean coefficient undefined.  At fixed perturbative order the vanishing
channel arrangement can instead be resolved into nested radial boundary faces.

\begin{theorem}[Resolved exceptional Euclidean factorization]
\label{thm:resolved-exceptional-euclidean}
Let $\beta:\widetilde{\mathcal K}\to\mathcal K$ be the finite iterated real
blow-up of the exceptional Euclidean channel strata meeting a declared compact
panel.  At fixed finite $(L,\Delta,r,N)$, every retained zero-floor coefficient
and source/BV/momentum derivative has, on each resolved chart, a
polyhomogeneous expansion
\begin{equation}
 \beta^*\partial_IW_{\infty,0}
 \sim
 ...
 \label{eq:exceptional-polyhomogeneous}
\end{equation}
Below every finite multiweight only finitely many terms occur.  Each
coefficient is a finite sum of universal soft-forest tensors times already
renormalized nonexceptional hard quotient coefficients or existing local
contact data.  Along a weighted approach $\rho_A=\varepsilon^{\nu_A}$,
\begin{equation}
 |\partial_IW_{\infty,0}(\varepsilon)|
 \le C_I\varepsilon^{\Lambda_*}
 (1+|\log\varepsilon|^{M_*}),
 \label{eq:exceptional-power-log}
\end{equation}
with the leading exponent improved whenever the complete Ward or representation
sum cancels its nominal leading terms.  No new renormalization coordinate is
introduced, and the common master/source identities and loop-order projectivity
hold coefficientwise in the expansion.
\end{theorem}
\begin{proof}[Proof mechanism]
Resolve compatible vanishing channel sums as a finite nested forest.  At the
innermost face, Taylor-expand the hard complement in the soft variables,
separate the soft subgraph, and contract it.  The quotient graph belongs to the
same contraction-complete hard packet.  Iterate outward through the forest;
equal radial exponents generate only finite powers of logarithms.  Uniqueness
of the resulting polyhomogeneous expansion makes the BV/source relations hold
coefficient by coefficient and shows that the hard factors are precisely the
already-owned nonexceptional or local coefficients.
\end{proof}
\src{PG44}{thm:v44-polyhomogeneous,thm:v44-power-log,prop:v44-hard-v43,thm:v44-close}

\scope{These are Euclidean coefficientwise theorems at fixed finite
perturbative/EFT/source order.  They do not provide an all-orders summation, an
analytic finite-coupling trajectory, or physical Minkowski boundary values.
After analytic continuation, genuine Landau pinch surfaces and soft--collinear
jet overlap require a separate factorization and inclusive/dressed analysis.}
...
The perturbative branch nevertheless closes several of these interfaces at a
strictly stronger finite-order level than an abstract formal expansion.
Theorem~\ref{thm:bosonic-egh-two-loop} gives one retained-constraint
Einstein--three-gauge-factor--Higgs shell state whose local ghost-one defect is
restored without erasing the independent weight-six EFT quotient, while
\eqref{eq:egh-composed-source} controls the composed source map through two
loops uniformly in the number of ultraviolet shell steps.  Theorems
\ref{thm:nonexceptional-zero-floor} and
\ref{thm:resolved-exceptional-euclidean} then separate the ordinary positive
renormalization point from an auxiliary massless floor and replace exceptional
Euclidean divergences by an explicit soft-forest boundary calculus.  These
results establish a coefficientwise perturbative continuum on their declared
finite packets; they do not provide the analytic finite-coupling trajectory
or the positive nonperturbative law required by Theorem~\ref{thm:main}.
...
A useful next result should therefore close a \emph{named quantitative row} on a \emph{single actual family}. Examples include a uniform rough chiral-line/reflection construction, a no-escape estimate for the positive reduced metric law, a common renormalized source comparison, or a nonzero separated excess with a controlled limit.  On the coefficientwise perturbative branch, the immediate extensions are now more specific: adjoin the chiral fermion/Yukawa determinant packet to the bosonic two-loop self-map, and construct physical Minkowski boundary values with a non-overcounting Landau soft--collinear factorization of the resolved Euclidean coefficients.  Neither extension is replaced by repeating finite algebraic identities with an added cutoff index.
...
The resulting mathematical position is therefore specific: a finite structural and dynamical reconstruction, a broad collection of exact analytic modules and obstruction theorems, a finite-order perturbative continuum with a two-loop bosonic Einstein--gauge--Higgs self-map and controlled Euclidean massless limits, a sufficient regular-branch nonperturbative continuum chain, and a set of explicit coupled sectors that test the physical source mechanism.  The perturbative achievements do not collapse the distinction between coefficientwise and analytic/nonperturbative completion.  The remaining achievement is to realize the full chain cofinally on one physical family, including the chiral line and physical massless boundary values, without replacing the clock or normalizing away the Einstein source.
...
Transport-derived geometry & Fixed finite coboundaries have zero Perron jet, while unbounded thermodynamic gradients can retain transport; unsigned and oriented $K_4$ edge modules are separated (Prop.~\ref{prop:finite-thermodynamic-coboundary}, Eq.~\eqref{eq:k4-edge-representation-fork}) & Same-record incidence $A_{\rm cell},A_{\rm skel},A_{\rm ext}$ and Green--Kubo match for the actual physical writer.\\[1pt]
...
Protected regulator fibre & Pure regulator projector topology and residual flat regulator character contract on the protected minimal overlap component (Thm.~\ref{thm:pure-regulator-contraction}) & Does not remove physical-base topology, Pfaffian/Real orientation, reflection positivity, or wall/crossing control.\\[1pt]
...
Perturbative bosonic EGH & Retained-constraint two-loop weight-six $SU(3)\times SU(2)\times U(1)_Y$--Higgs BV/source self-map with independent Wilson quotient and uniform composed source transport (Thm.~\ref{thm:bosonic-egh-two-loop}) & Fixed order/domain only; chiral fermions, Yukawa block, determinant/Pfaffian transport, all-orders trajectory and nonperturbative positivity remain separate.\\[1pt]
...
Perturbative Euclidean IR & Auxiliary massless floor removed on compact nonexceptional panels; UV/IR coefficient limits commute (Thm.~\ref{thm:nonexceptional-zero-floor}) & Fixed finite perturbative/EFT/source packet; exceptional and physical on-shell limits require separate treatment.\\[1pt]
Exceptional Euclidean channels & Resolved vanishing-channel strata have polyhomogeneous universal-soft $\times$ hard expansions with no new counterterm (Thm.~\ref{thm:resolved-exceptional-euclidean}) & Minkowski boundary values, Landau pinch surfaces, genuine collinear jets, and inclusive/dressed cancellations remain open.\\[1pt]
...
additional perturbative references P80, PG44, and PP40 denote, respectively,
the bosonic Einstein--gauge--Higgs finite-order branch through two loops, the
general coefficientwise ultraviolet/infrared and protected-regulator branch,
and the phenomenological transport/displacement branch.  Their results are
integrated at the points where their hypotheses enter the monograph rather
than treated as a separate appended development.  The
...
The bounded conformal Ward estimate is used as the supported Dirichlet result of Theorem~\ref{thm:supported-response}, not as a global inverse of the unrestricted Witten operator. The fixed-spacing Gibbs existence theorem retains the full common conformal mode and does not assert uniqueness or uniform ultraviolet control. Stable one-sided polynomial tilts replace absolute exponential-moment assumptions that fail for Gaussian polynomials of degree greater than two.  A fixed finite-state coboundary theorem is not used to erase transport from an unbounded thermodynamic embedding, and the unsigned and oriented six-edge $K_4$ representations are not identified by dimension.  On the perturbative branch, the positive renormalization point is separated from the auxiliary infrared floor.  The vanishing native bosonic gauge-anomaly projection in Theorem~\ref{thm:bosonic-egh-two-loop} is not promoted to a statement about the chiral fermion determinant line, and symmetry-restoring ghost-one coordinates are kept distinct from independent ghost-zero Wilson/EFT data. The elementary strong-mixing branch is allowed to Gaussianize; a degree-incomplete Gaussian stress comparator is not used as evidence of interaction. Fixed smooth gauge charts are separated from the support of the explicit heat-link projective law. Global-gap statements are restricted to the intended massive sector and are not imposed on a photon-containing endpoint.
...
\bibitem[P80]{P80} A.\ P\'elissier. \emph{Perturbative Einstein--Standard Model Effective Field Theory Continuum Programme}, cumulative bosonic continuation through V80. Research manuscript.
\bibitem[PG44]{PG44} A.\ P\'elissier. \emph{Perturbative Einstein--Standard Model Continuum: General Continuation}, V44. Research manuscript.
\bibitem[PP40]{PP40} A.\ P\'elissier. \emph{Perturbative Einstein--Standard Model Continuum: Phenomenological Continuation}, V40. Research manuscript.
\end{thebibliography}
\endgroup
\end{document}
