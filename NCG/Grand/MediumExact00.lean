...
/-!
# Medium-exact closures, batch 00 (Gran-Tensor manuscript)

Exact full renderings closing the named gaps of the following records:

* `cor:S4-provenance-isotypic` — the complete five-isotype `S₄` carrier:
...
Further records of the batch are appended below section by section.
-/
