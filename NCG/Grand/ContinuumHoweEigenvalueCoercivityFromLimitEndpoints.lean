...
/-!
# Continuum Howe coercivity from numerical eigenvalues and limiting endpoints

This companion to the compressed-resolvent compiler accepts the model's numerical first-positive
eigenvalues directly.  Norm convergence and two endpoint exclusions still discharge all contour
and Riesz-convergence inputs, while numerical gap convergence gives uniform coercivity.
-/
...
/-- A convergent sequence of positive spectral values, together with only limiting endpoint
separation, yields eventual exact graph kernels and an explicit uniform half-gap estimate. -/
theorem continuumHowe_kernel_and_uniformCoercivity_of_limit_endpoints
    ...
    (energy residual : ℕ → E → ℝ) (μ : ℕ → ℝ) (μlim : ℝ)
    (hresidual : ∀ n x, 0 ≤ residual n x)
    (hcoercive : ∀ n x, μ n * residual n x ≤ energy n x)
    (hμ : Tendsto μ atTop (𝓝 μlim)) (hμPos : 0 < μlim) :
    (∀ᶠ n in atTop,
      LinearMap.range (P n).toLinearMap = operatorGraphKernel (D n) (A n)) ∧
      0 < μlim / 2 ∧
      (∀ᶠ n in atTop, ∀ x, μlim / 2 * residual n x ≤ energy n x) ∧
      ∀ xseq : ℕ → E,
  ...
  exact continuumHowe_kernel_and_uniformCoercivity
    D A lam hlam T P hequation hcompact hsymmetric (center : ℂ) radius hzero hinside
      ...
      hRieszTendsto hprotected hprotectedRank energy residual μ μlim
      hresidual hcoercive hμ hμPos
