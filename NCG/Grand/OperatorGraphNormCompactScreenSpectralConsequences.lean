...
/-!
# Spectral consequences of compact graph-energy screens

The manuscript's full graph-energy screen hypothesis feeds directly into
the compact self-adjoint spectral package: a compact limit, operator-norm
resolvent convergence, exact Riesz eigenspaces, stable multiplicities,
norm-convergent projections, and finite projected source Gram convergence.
-/
...
/-- Full graph-energy screen tightness implies the complete automatic-circle
spectral approximation package. -/
theorem operatorGraphResolvent_fullSpectralConsequences_of_graphNormScreenTight
    {ι : Type x}
  ...
  exact J.compressedOperator_spectralConsequences_of_collectivelyCompact_automaticCircle
    Rn T hdense hstrong hcollective
      ...
      hlimSymm center hcenter
      source sourceLim hsource

omit [∀ n, FiniteDimensional ℂ (Hn n)] in
/-- A non-monotone compact-screen tail estimate on the full graph-energy
unit-ball union implies the complete automatic-circle spectral package. -/
theorem operatorGraphResolvent_fullSpectralConsequences_of_graphNormScreenTails
    {ι : Type x}
    (J : System (K := ℂ) (H := H) (Hn := Hn))
    (L : System (K := ℂ) (H := WithLp 2 (H × F))
      (Hn := fun n ↦ WithLp 2 (Hn n × Fn n)))
