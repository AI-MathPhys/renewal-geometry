...
/-!
# Exact medium-tier renderings, batch 06 (Gran-Tensor manuscript)

Closes the named audit gaps of the following DOWNGRADED records with genuine
finite-dimensional developments:

* `thm:SMST-finite-action-Einstein` — derived Ward divergence/pairing
...
* `thm:SMST-generator-projections` — Krylov saturation with a genuine
  orthogonal projection, the finite-Dirac projector as an honest
  Hilbert–Schmidt orthogonal projection onto a real relation space, the exact
  three-term budget, the provenance-zero identification, and the scalar
  commutant expectation;
* `thm:SMST-joint-writer-orientation` — whitened-overlap polar splitting,
  innovation identity, and the SMW.6 derivative layer;
* `thm:SMST-line-valued-Einstein` — genuine finite line-bundle data (edge
  transports, walk holonomy, gradient theorem on connected graphs, amplitude
  polar splitting) and the covariant residual with derived sources;
* `thm:SMST-matter-Legendre` — general non-quadratic Legendre reciprocity,
  the measured multi-species matter bracket, the gravity–matter cross
  anomaly, the closure certificates, and the CAR lift;
* `thm:SMST-quiver-commutant` — the composed central-decomposition reduction
  identifying the ambient commutant with `End 𝔔`;
* `thm:SMST-reducing-coherent-closure` — the assembled Loewner envelope
  bound, the `τ⁻¹ log W_t → 0` step scale, and the final perturbed Schur
  estimate;
* `thm:SMST-support-polar-commutant` — the C*-generation clause: all polar
  data are polynomial functional-calculus outputs of the incidence maps, so
  the two generated star algebras coincide;
* `thm:SMST-unit-writer-dimension` — the census writer dimensions computed
  as finranks of genuine intertwiner spaces of an explicit signature table;
* `thm:SMST-zero-mode-saturation` — invertibility of the off-kernel
  restriction and the canonical wedge factorization of the determinant line.

`thm:SMST-loaded-emergence` is deliberately not rendered here; see the batch
report.
-/
  ...
  have hpair : ∀ u : Fin ng → ℝ,
    ...
    have hF : HasDerivAt
      ...
      have hS' : HasFDerivAt S L (A₀ + (0 : ℝ) • (D *ᵥ u), flow u 0) := by
        rw [hflow0 u]
        simpa using hS
      simpa using hS'.comp_hasDerivAt 0 hc
    ...
    have hrepr : L (D *ᵥ u, R *ᵥ u)
      ...
      rw [hL, add_dotProduct, Matrix.mulVec_transpose,
        Matrix.mulVec_transpose, vecMul_dotProduct_comm,
        vecMul_dotProduct_comm, dotProduct_mulVec, dotProduct_mulVec]
    rw [← hrepr, hzero]
