...
/-!
# GRH integrated zero mode and sharp-chain central bank
...
namespace NCG
namespace GRHIntegratedZeroModeAndSharpCentralBank
...
theorem router_dispersion_complex {ι : Type*} [Fintype ι] [Nonempty ι]
  ...
  have hIK : RCLike.re (⟪∑ i, b i, K⟫_ℂ)
    ...
    simp [Complex.mul_re, Complex.normSq, hA.ne', div_eq_mul_inv]
    field_simp
    norm_cast
...
noncomputable abbrev fixedSpace (T : E →L[ℂ] E) : Submodule ℂ E :=
  LinearMap.ker (((1 : E →L[ℂ] E) - T).toLinearMap)
...
theorem centralGram_inner (V : ∀ j, E →ₗᵢ[ℂ] Ej j)
  ...
  change (((bWeight m j : ℝ) : ℂ) * ((‖rowRead V T j g‖ : ℝ) : ℂ) ^ 2).re
    = bWeight m j * ‖rowRead V T j g‖ ^ 2
  have hnre : ((((‖rowRead V T j g‖ : ℝ) : ℂ) ^ 2).re)
      = ‖rowRead V T j g‖ ^ 2 := by norm_cast
  have hnim : ((((‖rowRead V T j g‖ : ℝ) : ℂ) ^ 2).im) = 0 := by norm_cast
  rw [Complex.mul_re, Complex.ofReal_re, Complex.ofReal_im, hnre, hnim]
  ring
...
theorem centralGram_isPositive (V : ∀ j, E →ₗᵢ[ℂ] Ej j)
  ...
  classical
  let term : Fin m → E →L[ℂ] E := fun j =>
    ((bWeight m j : ℝ) : ℂ) •
      (ContinuousLinearMap.adjoint (rowRead V T j) ∘L rowRead V T j)
  have hterm : ∀ j, (term j).IsPositive := by
    intro j
    exact (ContinuousLinearMap.isPositive_adjoint_comp_self (rowRead V T j)).smul_of_nonneg
      (by exact_mod_cast bWeight_nonneg m j)
  have hsum : ∀ s : Finset (Fin m), (∑ j ∈ s, term j).IsPositive := by
    intro s
    induction s using Finset.induction_on with
    | empty => simpa using (ContinuousLinearMap.isPositive_zero (𝕜 := ℂ) (E := E))
    | @insert j s hjs ih =>
        rw [Finset.sum_insert hjs]
        exact (hterm j).add ih
  simpa only [centralGram, term] using hsum (SharpChain.centralSet m)
...
theorem sqrt_centralMass_lipschitz (y z : PiLp 2 Ej) :
  ...
  have htri : Real.sqrt (centralMassOf y)
    ...
    have hnorm : ∀ w : PiLp 2 Ej, ‖R w‖ = Real.sqrt (centralMassOf w) := by
      intro w
      simp [PiLp.norm_eq_of_L2, centralMassOf, R]
      simpa only [Finset.univ_eq_attach] using congrArg Real.sqrt
        (Finset.sum_attach (SharpChain.centralSet m) fun j => ‖w.ofLp j‖ ^ 2)
...
end GRHIntegratedZeroModeAndSharpCentralBank
end NCG
