...
/-!
# Finite compound matrices as exterior powers

Shared finite-dimensional machinery for the SMQG exterior-power cluster.  The
finite rendering realizes the exterior power `⋀^r E` of `E = ℂ^d` concretely on the
...
namespace NCG
namespace FiniteCompoundMatrixExteriorPower
...
theorem conjTranspose_asym_mul_asym :
  ...
  simp only [Matrix.conjTranspose_apply, asym_apply, star_sum, star_intCast,
    star_zero, Finset.sum_mul_sum]
  have hstar : ∀ (f : Fin r → Fin d) (σ : Equiv.Perm (Fin r)),
      star (if f = sel S ∘ σ then ((Equiv.Perm.sign σ : ℤ) : ℂ) else 0) =
        if f = sel S ∘ σ then ((Equiv.Perm.sign σ : ℤ) : ℂ) else 0 := by
    intro f σ
    by_cases h : f = sel S ∘ σ <;> simp [h]
  simp_rw [hstar]
  have hterm : ∀ f : Fin r → Fin d, ∀ σ τ : Equiv.Perm (Fin r),
    ...
    · by_cases h2 : f = sel T ∘ τ
      · simp [h1, h2, h1.symm.trans h2]
      · have h12 : ¬sel S ∘ σ = sel T ∘ τ := fun h => h2 (h1.trans h)
        simp [h1, h2, h12]
    · simp [h1]
  rw [Finset.sum_comm]
  rw [Finset.sum_congr rfl fun σ _ => Finset.sum_comm]
  rw [Finset.sum_congr rfl fun σ _ =>
    Finset.sum_congr rfl fun τ _ =>
      Finset.sum_congr rfl fun f _ => hterm f σ τ]
  ...
  · subst hST
    ...
    have hsum : ∀ σ : Equiv.Perm (Fin r),
      ...
      · intro τ _ hτ
        rw [if_neg fun hc => hτ ((hdiag σ τ).mp hc).symm]
      · intro h; exact absurd (Finset.mem_univ σ) h
