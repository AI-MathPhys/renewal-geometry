...
/-!
# Canonical source fusion

Exact full-statement formalization closing the audit gap for one record:
...
namespace NCG
namespace CanonicalSourceFusion
...
theorem gram_isUnit_det_of_surjective (B : Matrix N K ℂ)
  ...
  have hBv : Bᴴ *ᵥ v = 0 := by
    have hself : star (Bᴴ *ᵥ v) ⬝ᵥ (Bᴴ *ᵥ v) = 0 := by
      rw [gram_realization_inner, Matrix.conjTranspose_conjTranspose,
        hv, dotProduct_zero]
    exact dotProduct_star_self_eq_zero.mp hself
  -- and `v` is orthogonal to the whole (surjective) range of `B`
  obtain ⟨u, hu⟩ := hB v
  change B *ᵥ u = v at hu
  have hvv : star v ⬝ᵥ v = 0 := by
    calc
      star v ⬝ᵥ v = star v ⬝ᵥ (B *ᵥ u) := by
        rw [hu]
      _ = (star v ᵥ* B) ⬝ᵥ u := by rw [Matrix.dotProduct_mulVec]
      _ = star (Bᴴ *ᵥ v) ⬝ᵥ u := by
        rw [Matrix.star_mulVec, Matrix.conjTranspose_conjTranspose]
      _ = 0 := by rw [hBv]; simp
  exact hv0 (dotProduct_star_self_eq_zero.mp hvv)
