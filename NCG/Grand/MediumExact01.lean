...
/-!
# Medium exact batch 01

Exact full renderings closing the audited gaps of five Gran-Tensor records:

* `thm:minimal-record-audit` (`NCG.RecordAuditExact`): the partition-refinement
  algorithm is formalized on an actual finite record automaton — the `k`-round
  refinement `eqv` identifies exactly the records not separated by a future
  word of length at most `k` (P1), stabilizes with every strict round below
  `n_rec - 1` (P2), the stable partition is full word equivalence (P3), the
  boxed signature residual `‖v(r) - v(s)‖² = 0` characterizes it (P4), and the
  positive minimum nonzero signature distance gives exact robust recovery
  whenever `4ε < δ_rec` (P5).
* `thm:interface-coercivity` (`NCG.FeshbachInterface`): both boxed displays in
  genuine `ℓ²` operator norms — the Schur-response floor
  `s_min(Λ) ≥ δ_A - κ²/δ_I`, and the full network resolvent bound
  `‖(𝔸 - z)⁻¹‖ ≤ M_D + (1 + M_D²M_B²)/δ_E`, with the Feshbach block inverse
  *derived* from the exterior/interior split rather than assumed.
* `thm:fresh-endpoint-retained-record` (`NCG.HelstromRetainedRecord`): the
  completed-private reset channel (imported), plus the Helstrom *optimality*
  derived over all two-outcome effects: the equal-prior success probability of
  every measurement is at most `(5 + 2√6)/10`, attained by the explicit
  spectral projector of the state difference.
* `thm:gap-soft-mode-alternative` (`NCG.MoscoGapSoftMode`): the Mosco layer is
  formalized — recovery sequences and weak lower semicontinuity define Mosco
  convergence, the boxed passage holds with a genuine `Filter.liminf`, soft
  modes are extracted from collapsing *optimal* transient Rayleigh constants,
  and core witnesses carry finite source-Gram residual panels.
* `thm:matrix-continued-fraction` (`NCG.JacobiChainExact`): the block Jacobi
  matrix `𝕁` is constructed, its tail responses are defined as inverse
  resolvent compressions, the boxed forward recursion
  `Λ_k = zI - C_k - B_kᴴ Λ_{k+1}⁻¹ B_k` and base `Λ_m = zI - C_m` are *derived*
  from `𝕁` by Schur complementation, the reverse stripping formula follows,
  and in the positive gauge the complete imaginary-ray response uniquely
  reconstructs every layer of a Hermitian chain.
-/
...
theorem exists_stable_round :
  ...
  | inr hne =>
      ...
      have hstep : ∀ k ≤ n - 1,
        ...
        intro k hk
        exact card_quotient_lt_of_strict out u (hcon k hk) _ _ rfl rfl
      have hgrow : ∀ k ≤ n, k + 1 ≤ Nat.card (Quotient (eqvSetoid out u k)) := by
        ...
        | zero =>
            ...
            simpa [Nat.succ_le_iff, Nat.card_pos_iff] using
              ⟨this, Finite.of_fintype _⟩
        | succ k ih =>
            intro hk
