...
import NCG.Topology.Brouwer.CubicalSpernerPrep
...
theorem strong_cubical_sperner (k: ℕ ) : ∀ (SC : SpernerCube), k = SC.n →
  ...
  }
  {
    ...
    letI completeSimplexDecidable :
        DecidablePred (fun I : Fin (SC2.n + 1) → SC2.G =>
          complete_simplex SC2 SC2.n I) := Classical.decPred _
    letI completeSimplexDecidableOnParentIndex :
        DecidablePred (fun I : Fin (k + 1) → SC2.G =>
          complete_simplex SC2 SC2.n I) := Classical.decPred _
    ...
    have hcomp I : complete_boundary_face SC1 (f2 I) ↔ complete_simplex SC2 SC2.n I := by {
      ...
      }
      {
        ...
        {
          intro h4 j
          change (I (Fin.ofNat (k + 1) i) j).1 ≤
              (I (Fin.ofNat (k + 1) (i + 1)) j).1 ∧
            (I (Fin.last k) j).1 ≤ (I 0 j).1 + 1
    ...
    {
      intro I hI
      have hI' : complete_simplex SC2 SC2.n I := by
        have hmem : I ∈ Finset.univ.filter (fun I => complete_simplex SC2 SC2.n I) := by
          exact Finset.mem_coe.mp hI
        exact (Finset.mem_filter.mp hmem).2
      apply Finset.mem_coe.mpr
      show f2 I ∈ Finset.univ.filter (fun J => complete_boundary_face SC1 J)
      exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, (hcomp I).mpr hI'⟩
    }
    {
    ...
    }
    {
      ...
      apply Finset.mem_coe.mpr
      show I ∈ Finset.univ.filter (fun I => complete_simplex SC2 SC2.n I)
      exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, (hcomp I).mp (h4 ▸ h3)⟩
    }
  }
